...
Sobre custo: o Gemini tem camada gratuita, mas **só para contas pessoais (@gmail.com) — contas do Google Workspace (domínio próprio, tipo @agricef.com.br) não têm direito a ela** e mostram cota zerada até você vincular uma forma de pagamento ao projeto (em [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey), clique em "Configurar faturamento" ao lado da chave). Uma vez com faturamento ativo, o custo por uso, para o volume de uso daqui (um resumo curto por clique), tende a ficar bem baixo. Se o modelo `gemini-3.5-flash-lite` (constante `GEMINI_MODEL` no topo do `Code.gs`) for descontinuado ou restringido no futuro — a Google costuma aposentar/restringir modelos com bastante frequência, já trocamos esse nome duas vezes durante a configuração —, troque o nome ali por um modelo atual: veja a lista em [ai.google.dev/gemini-api/docs/models](https://ai.google.dev/gemini-api/docs/models). Se o erro mencionar "no longer available to new users" mesmo com o modelo mais recente da lista, é sinal de que esse modelo específico está sendo descontinuado para chaves novas — tente a próxima geração (ex.: se `gemini-3.5-*` parar, tente `gemini-4.0-*` se existir).
